...
public class OverWorld implements TileBasedMap2d {
    ...
    public void setRealmId(int globalX, int globalY, int settlementId) {
        OverWorldChunk chunk = loadGlobalChunk(globalX, globalY);
        if (chunk != null)
            chunk.realmIds[getPrecise(globalX)][getPrecise(globalY)] = settlementId;
    }

    public int getRealmId(int globalX, int globalY) {
        OverWorldChunk chunk = loadChunk(getChunkCoord(globalX), getChunkCoord(globalY));
        if (chunk == null) {
            return -2;
        }
        return chunk.realmIds[getPrecise(globalX)][getPrecise(globalY)];
    }
}
